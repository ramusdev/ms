...
class BobTest extends PHPUnit\Framework\TestCase
{
    ...
    // Custom
    public function testShoutQuestionNumeralsCustom()
    { 
        $this->assertEquals(
            "Calm down, I know what I'm doing!",
            $this->bob->respondTo("WHY DID YOU USE 3.2 FOR PI?")
        );
    }

    public function testShoutNumeralsSpecCustom()
    {
        $this->assertEquals("Whoa, chill out!", $this->bob->respondTo("O!"));
    }

    public function testShoutEndingDotCustom() {
        $this->assertEquals("Whoa, chill out!", $this->bob->respondTo("UPSIDE DOWN."));
    }
    
    public function testTextCustom()
    {
        $this->assertEquals("Whatever.", $this->bob->respondTo("NYC is New York City"));
    }
